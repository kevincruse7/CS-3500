...
abstract class AbstractDuration implements Duration {
  ...
  public String format(String template) {
    ...
    int prevIndex = 0;
    int index = 0;
    while (template.indexOf('%', index) != -1 && index < template.length()) {
      ...
      result.append(template, prevIndex, index);

      switch (template.charAt(index + 1)) {
        case 't':
          result.append(inSeconds());
          break;
        case 'h':
          result.append(hoursOf(inSeconds()));
          break;
        case 'H':
          result.append(String.format("%02d", hoursOf(inSeconds())));
          break;
        case 'm':
          result.append(minutesOf(inSeconds()));
          break;
        case 'M':
          result.append(String.format("%02d", minutesOf(inSeconds())));
          break;
        case 's':
          result.append(secondsOf(inSeconds()));
          break;
        case 'S':
          result.append(String.format("%02d", secondsOf(inSeconds())));
          break;
        case '%':
          result.append('%');
          break;
